...
const personalize = (text, m) => String(text || "").replaceAll("{{naam}}", firstName(m.full_name)).replaceAll("{{name}}", firstName(m.full_name));
  ...
  for (const part of chunk(targets, 100)) {
    const payload = part.map((m) => {
      const rewardLine = c.reward_credits > 0
        ...
        : "";
      const body = rewardLine + personalize(c.body_html, m);
      return {
        from: FROM_NEWS,
        to: m.email,
        replyTo: REPLY_TO,
        subject: personalize(c.subject || c.name, m),
        html: newsletterHtml({ subject: personalize(c.subject, m), preheader: c.preheader, body, unsubUrl: `${SITE}/uitschrijven?token=${m.unsub_token}` }),
