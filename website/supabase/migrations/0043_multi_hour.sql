-- 0043: multi-hour bookings (book 1–4 consecutive hours) + duration discount.
-- A session is 60 min; you can chain hours. Exclusivity is enforced via an overlap check
-- (plus the existing per-start unique index against concurrent same-start double-booking).

-- gym_taken_slots now expands every confirmed booking into each hour it covers, so the UI greys
-- out all occupied hours (not just the start).
...
language sql stable security definer set search_path = public as $$
  ...
  from bookings b
  cross join lateral generate_series(b.starts_at, b.ends_at - interval '1 minute', interval '1 hour') gs
...
-- Recreate create_booking with p_hours (consecutive hours).
...
begin
  ...
  -- Exclusivity over the whole range (confirmed bookings + blocks).
  ...
  -- duration discount: 2u −5%, 3u −8%, 4u −10%.
