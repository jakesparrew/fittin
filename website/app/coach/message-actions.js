...
export async function sendMessage(formData) {
  ...
  const other = user.id === coachId ? clientId : coachId;
  await notify({ gymId: me.gym_id, userId: other, actorId: user.id, type: "system", title: `Nieuw bericht van ${me.full_name || "je " + (user.id === coachId ? "coach" : "client")}`, body: body.slice(0, 80), link: user.id === coachId ? "/training" : "/coach/berichten" });
