...
export default async function CoachClienten() {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      ...
      <h1 className="mt-2 text-3xl font-black text-ink">Mijn clienten</h1>
      <p className="mt-1 text-sm text-ink/50">Verbind je met leden en volg hun vooruitgang. Enkel verbonden clienten kan je boeken.</p>
      ...
      {voorstellen.length > 0 && (
        <div className="mt-6 rounded-3xl border-2 border-amber-300 bg-surface p-6">
          ...
          <p className="mt-1 text-sm text-ink/60">Deze mensen vroegen een intake aan bij Fittin&rsquo;. Jij beslist of je ze overneemt.</p>
          <div className="mt-3 space-y-3">
            {voorstellen.map((r) => (
              <div key={r.id} className="rounded-2xl border border-borderc p-4">
                <p className="font-bold text-ink">{r.client_name || r.client_email}</p>
                <p className="text-xs text-ink/45">{r.client_email}</p>
                {r.note && <p className="mt-2 rounded-xl bg-paper px-3 py-2 text-xs text-ink/60">{r.note}</p>}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <ActionForm action={beantwoordDoorgave} success="Geweigerd ✓">
                    ...
                    <button className="rounded-full border-2 border-borderc px-4 py-2 text-xs font-bold text-ink transition hover:border-red-300 hover:text-red-600">Weigeren</button>
                  </ActionForm>
                </div>
              </div>
            ))}
      ...
      {/* Connect a client */}
      <div className="mt-6 rounded-3xl border-2 border-accent bg-surface p-6 shadow-sm shadow-accent/10">
        <h2 className="text-lg font-black text-ink">Verbind een client</h2>
        <p className="mt-1 text-sm text-ink/60">Stuur een verbindingsverzoek naar een lid. Zij krijgen een melding om te aanvaarden — pas daarna verschijnt de client hieronder en kan je sessies met hen boeken. Een lid kan jou ook zelf aanvragen.</p>
        {connectable.length > 0 ? (
          ...
          <p className="mt-3 text-sm text-ink/50">Alle leden zijn al verbonden of uitgenodigd.</p>
        )}
        <AddClientInline />
        <p className="mt-2 text-xs text-ink/40">Nieuw lid zonder account? Maak het hierboven aan via e-mail — het wordt meteen aan jou verbonden en krijgt een login-uitnodiging.</p>
      </div>

      {/* Incoming connection requests (a member asked to be coached by me) */}
      {incoming.length > 0 && (
        <div className="mt-6 rounded-3xl border border-borderc bg-surface p-6">
          <h2 className="flex items-center gap-2 text-lg font-black text-ink">Verzoeken om verbinding <span className="rounded-full bg-accent/15 px-2 py-0.5 text-xs text-accentdark">{incoming.length}</span></h2>
          <p className="mt-1 text-sm text-ink/50">Deze leden willen door jou gecoacht worden.</p>
          <div className="mt-3 space-y-2">
            {incoming.map((l) => (
              <div key={l.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-borderc p-4">
                <div><p className="font-bold text-ink">{l.client.full_name || l.client.email}</p><p className="text-xs text-ink/45">{l.client.email}</p></div>
                <div className="flex gap-2">
                  ...
                  <ActionForm action={respondCoachLink} success="Bijgewerkt ✓"><input type="hidden" name="linkId" value={l.id} /><input type="hidden" name="accept" value="0" /><button className="rounded-full border-2 border-borderc px-4 py-2 text-xs font-bold text-ink transition hover:border-red-300 hover:text-red-600">Weigeren</button></ActionForm>
                </div>
              </div>
            ))}
          </div>
      ...
      {sent.length > 0 && (
        <div className="mt-6 rounded-3xl border border-borderc bg-surface p-6">
          <h2 className="text-lg font-black text-ink">Verzonden uitnodigingen</h2>
          <p className="mt-1 text-sm text-ink/50">Wachten tot het lid je verbinding aanvaardt.</p>
          <div className="mt-3 space-y-2">
            {sent.map((l) => (
              <div key={l.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-borderc p-4">
                <div><p className="font-bold text-ink">{l.client.full_name || l.client.email}</p><p className="text-xs text-ink/45">{l.client.email}</p></div>
                <div className="flex items-center gap-3">
                  <span className="rounded-full bg-paper px-3 py-1 text-xs font-bold text-ink/50">In afwachting</span>
                  <ActionForm action={removeCoachLink} success="Verwijderd ✓"><input type="hidden" name="linkId" value={l.id} /><button className="rounded-full border-2 border-borderc px-4 py-1.5 text-xs font-bold text-ink transition hover:border-red-300 hover:text-red-600">Annuleer</button></ActionForm>
                </div>
              </div>
            ))}
          </div>
      ...
      {wachtOpAccount.map((r) => (
        <div key={r.id} className="mt-6 rounded-2xl border border-borderc bg-surface p-4">
          <p className="font-bold text-ink">Aanvaard, wacht op het account van {r.client_name || r.client_email}</p>
          ...
          <p className="mt-1 text-xs text-ink/40">{r.client_email}</p>
        </div>
      ))}

      {/* Connected clients */}
      <h2 className="mt-8 text-xl font-black text-ink">Verbonden clienten</h2>
      {accepted.length === 0 ? (
        <div className="mt-3 rounded-3xl border border-dashed border-borderc bg-surface p-10 text-center">
          <p className="font-semibold text-ink/70">Nog geen verbonden clienten.</p>
          <p className="mt-1 text-sm text-ink/50">Verbind hierboven een lid, of laat een lid jou aanvragen via je coachprofiel.</p>
        </div>
      ) : (
        <div className="mt-3 grid gap-4 lg:grid-cols-2">
          {accepted.map((l) => {
            ...
            return (
              <div key={l.id} className="rounded-2xl border border-borderc bg-surface p-6">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="flex flex-wrap items-center gap-2 text-lg font-black text-ink">
                      {c.full_name || c.email}
                      {/* Chip zonder bedrag in beeld: het bedrag staat in de titel, zodat de kaart rustig blijft. */}
                      {doorgave && (
                        <span title={`Door Fittin' doorgegeven — ${feeZin(doorgave.fee_cents)}`} className="rounded-full bg-lav/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ink/60">
                          via Fittin&rsquo;
                        </span>
                      )}
                    </p>
                  ...
                  <span className={"rounded-full px-3 py-1 text-xs font-bold " + ((weekCount[c.id] || 0) > 0 ? "bg-accent/15 text-accentdark" : "bg-paper text-ink/50")}>
                ...
                {next && <p className="mt-3 text-xs capitalize text-ink/50">Volgende sessie: {fmt(next.starts_at)} · {next.services?.name}</p>}

                {/* Favorieten: waar bouw je een schema rond dat hij ook echt gaat doen? */}
                {(favByClient[c.id] || []).length > 0 && (
                  <p className="mt-3 text-xs text-ink/60">
                    <span className="font-bold text-ink/45">Doet graag:</span>{" "}
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <ActionForm action={removeCoachLink} success="Verwijderd ✓">
                    ...
                    <button className="rounded-full border-2 border-borderc px-4 py-2 text-xs font-bold text-ink transition hover:border-red-300 hover:text-red-600">Verbreek verbinding</button>
                  </ActionForm>
                </div>
                ...
                <div className="mt-4 flex flex-wrap items-end gap-2 border-t border-borderc pt-4">
                  <ActionForm action={setClientPrice} success="Tarief opgeslagen ✓" className="flex items-end gap-2">
                    ...
                    <button className="rounded-full bg-paper px-3 py-1.5 text-xs font-bold text-ink">Opslaan</button>
                  </ActionForm>
                  <p className="pb-2 text-xs text-ink/40">Notitie voor jezelf — je client betaalt je rechtstreeks (bv. Bancontact), niet via het platform.</p>
                </div>
              </div>
            );
          })}
