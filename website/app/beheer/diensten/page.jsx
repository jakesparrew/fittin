...
export default async function Diensten() {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      <h1 className="text-3xl font-black text-ink">Diensten &amp; prijzen</h1>
      <p className="mt-1 text-sm text-ink/50">Pas prijzen, duur en capaciteit aan. Prijzen in euro.</p>
      ...
      <div className="mt-6 space-y-4">
        {(services || []).map((s) => (
          <form
            ...
            className="grid items-end gap-3 rounded-2xl border border-borderc bg-surface p-5 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,.7fr)_minmax(0,.7fr)_minmax(0,.7fr)_auto]"
          >
            ...
            <Col label="Status">
              <span className={"inline-block rounded-full px-3 py-1 text-xs font-bold " + (s.active ? "bg-accent/15 text-accentdark" : "bg-paper text-ink/40")}>
                {s.active ? "Actief" : "Uit"}
              </span>
            </Col>
      ...
      <div className="mt-6 space-y-2">
        {(services || []).map((s) => (
          <form key={s.id} action={toggleService} className="inline-block">
            <input type="hidden" name="id" value={s.id} />
            <input type="hidden" name="active" value={s.active ? "true" : "false"} />
            <button className="mr-2 rounded-full border-2 border-borderc px-4 py-2 text-xs font-bold text-ink transition hover:border-lav">
      ...
      <h2 className="mt-12 text-2xl font-black text-ink">Kortingscodes &amp; acties</h2>
      <p className="mt-1 text-sm text-ink/50">Maak promoties om je gym beter te vullen — leden gebruiken de code bij het afrekenen.</p>
      ...
      <ActionForm action={createDiscount} success="Kortingscode aangemaakt ✓" className="mt-4 flex flex-wrap items-end gap-3 rounded-2xl border border-borderc bg-surface p-5">
        ...
        <label className="flex items-center gap-2 pb-2.5 text-xs font-bold text-ink/70">
      ...
      <div className="mt-4 space-y-2">
        {(codes || []).map((c) => (
          <div key={c.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-borderc bg-surface p-4">
            <div>
              <p className="font-black text-ink">{c.code} <span className="ml-1 text-accentdark">{c.amount_cents != null ? `−€ ${(c.amount_cents / 100).toFixed(2).replace(".", ",")}` : `−${c.percent}%`}</span></p>
            ...
            <div className="flex items-center gap-2">
              <span className={"rounded-full px-3 py-1 text-xs font-bold " + (c.active ? "bg-accent/15 text-accentdark" : "bg-paper text-ink/40")}>{c.active ? "Actief" : "Uit"}</span>
              <ActionForm action={toggleDiscount} success="Bijgewerkt ✓"><input type="hidden" name="id" value={c.id} /><input type="hidden" name="active" value={c.active ? "true" : "false"} /><button className="rounded-full border-2 border-borderc px-3 py-1 text-xs font-bold text-ink">{c.active ? "Zet uit" : "Zet aan"}</button></ActionForm>
        ...
        {(!codes || codes.length === 0) && <p className="text-sm text-ink/50">Nog geen kortingscodes.</p>}
      </div>
    </div>
  );
}
