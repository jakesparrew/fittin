...
export async function grantCoachCredits(formData) {
  ...
  // Stripe-webhook — dus pas ná betaling.
  const gratis = String(formData.get("betaling") || "betaald") === "gratis";
  let resolvedNote = "";
  if (delta > 0 && !gratis) {
  ...
  revalidatePath("/beheer");
  revalidatePath("/coach");
  return { ok: true, message: `Sessietegoed bijgeschreven ✓${resolvedNote}` };
}
...
// afgeschaft op 2026-08-07. Ze schreef tegoed bij vóór er betaald was, waardoor een coach kon
// trainen op sessies die nooit betaald raakten. Coaches kopen nu vooraf (Stripe, tegoed komt pas
// na betaling binnen via de webhook) of betalen aan de gym, waarna de beheerder het bijschrijft
// met grantCoachCredits.
