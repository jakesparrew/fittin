...
export default async function Subscribers() {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      ...
      <h1 className="mt-2 text-3xl font-black text-ink">Abonnees</h1>
      <p className="mt-1 text-sm text-ink/50">{active} actief · {unsub} uitgeschreven · {(subs || []).length} totaal. Elk lid wordt automatisch toegevoegd.</p>

      <ActionForm action={addSubscriber} success="Abonnee toegevoegd ✓" className="mt-6 flex flex-wrap items-end gap-2 rounded-2xl border border-borderc bg-surface p-4">
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-borderc bg-surface">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-borderc">
            {(subs || []).map((s) => (
              <tr key={s.id}>
                <td className="px-5 py-3 font-semibold text-ink">{s.email}</td>
                <td className="px-5 py-3 text-ink/60">{s.name || "—"}</td>
                <td className="px-5 py-3"><span className="rounded-full bg-paper px-2.5 py-0.5 text-xs font-bold text-ink/60">{SOURCE[s.source] || s.source}</span></td>
                <td className="px-5 py-3">
                  {s.status === "active" ? <span className="text-xs font-bold text-accentdark">Actief</span> : <span className="text-xs font-bold text-ink/40">{s.status === "unsubscribed" ? "Uitgeschreven" : "Bounced"}</span>}
                </td>
                <td className="px-5 py-3 text-xs text-ink/40">{fmt(s.created_at)}</td>
                <td className="px-5 py-3 text-right">
                  <ActionForm action={setSubscriberStatus} success="Bijgewerkt ✓" className="inline">
                    ...
                    <button className="text-xs font-bold text-ink/50 hover:text-ink">{s.status === "active" ? "Uitschrijven" : "Heractiveren"}</button>
                  </ActionForm>
                </td>
              </tr>
            ))}
            {(!subs || subs.length === 0) && <tr><td colSpan={6} className="px-5 py-8 text-center text-sm text-ink/40">Nog geen abonnees.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
