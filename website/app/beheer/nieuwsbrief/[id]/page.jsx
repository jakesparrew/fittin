...
export default async function CampaignDetail({ params }) {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      <Link href="/beheer/nieuwsbrief" className="text-sm font-semibold text-ink/50 hover:text-ink">← Campagnes</Link>
        ...
        <>
          ...
          {c.status === "draft" ? (
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
              <ActionForm action={updateNewsletter} success="Opgeslagen ✓" className="rounded-2xl border border-borderc bg-surface p-6">
                <input type="hidden" name="id" value={c.id} />
                <h2 className="font-black text-ink">Opstellen</h2>
                ...
                <label className="mt-3 block">
                  <span className="mb-1 block text-sm font-bold text-ink">Bericht</span>
              ...
              <div className="rounded-2xl border border-borderc bg-surface p-6">
                <h2 className="font-black text-ink">Voorbeeld & verzenden</h2>
                <div className="mt-3 rounded-xl border border-borderc bg-paper p-4 text-sm text-ink/80">
                  <p className="font-bold text-ink">{c.subject || <span className="text-ink/40">— geen onderwerp —</span>}</p>
                  <div className="mt-2 whitespace-pre-wrap text-ink/70">{(c.body_html || "Nog geen bericht.").replace(/<[^>]+>/g, "")}</div>
                </div>
                <p className="mt-4 text-sm text-ink/60">Wordt verzonden naar <span className="font-black text-ink">{subCount || 0}</span> actieve abonnees vanaf <span className="font-semibold">nieuwsbrief@news.fittin.be</span>.</p>
                <SendNewsletterButton id={c.id} count={subCount || 0} />
              </div>
            </div>
          ) : (
            <div className="mt-6 rounded-2xl border border-borderc bg-surface p-6">
              <h2 className="font-black text-ink">Bericht</h2>
              <p className="mt-1 text-sm font-bold text-ink">{c.subject}</p>
              ...
              <iframe title="nieuwsbrief" sandbox="" srcDoc={c.body_html || ""} className="mt-2 h-[460px] w-full rounded-lg border border-borderc bg-surface" />
              <p className="mt-4 text-xs text-ink/40">Verzonden {fmt(c.sent_at)}.</p>
            </div>
          )}
        </>
      )}
      ...
      {isDrip && (
        <>
          <div className="mt-6 flex flex-wrap items-center gap-3 rounded-2xl border border-borderc bg-surface p-5">
            ...
            <div className="ml-auto flex items-center gap-2">
              {c.status !== "active" ? (
                ...
                <ActionForm action={setDripStatus} success="Bijgewerkt ✓"><input type="hidden" name="id" value={c.id} /><input type="hidden" name="status" value="paused" /><button className="rounded-full bg-paper px-4 py-2 text-sm font-bold text-ink/70">Pauzeren</button></ActionForm>
              )}
            </div>
          </div>
          ...
          {steps.length > 0 && enrolled > 0 && (
            <div className="mt-6 rounded-2xl border border-borderc bg-surface p-5">
              <p className="font-black text-ink">Voortgang door de reeks</p>
              <p className="mt-0.5 text-xs text-ink/50">Hoeveel ingeschrevenen elke stap (mail) al ontvingen.</p>
                ...
                {steps.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="flex items-center gap-3">
                      <span className="w-10 shrink-0 text-xs font-bold text-ink/50">#{s.step_no}</span>
                      ...
                      <span className="w-10 shrink-0 text-right text-xs font-bold text-ink/40">{w}%</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="mt-6 space-y-3">
            ...
            {steps.length === 0 && <p className="rounded-xl bg-paper p-4 text-sm text-ink/50">Nog geen stappen. Voeg hieronder de eerste mail toe.</p>}
          </div>

          <ActionForm action={addDripStep} success="Stap toegevoegd ✓" className="mt-4 rounded-2xl border border-dashed border-borderc bg-surface p-5">
            <input type="hidden" name="campaignId" value={c.id} />
            <p className="font-black text-ink">Stap toevoegen</p>
  ...
  if (bare) {
    return (
      <div>
        ...
        <p className="text-xl font-black text-ink">{value ?? 0}</p>
      </div>
    );
  }
  return (
    <div className="rounded-2xl border border-borderc bg-surface p-5">
      ...
      <p className={"mt-2 text-2xl font-black " + (accent ? "text-accentdark" : "text-ink")}>{value ?? 0}</p>
      {sub && <p className="text-xs font-semibold text-ink/40">{sub}</p>}
    </div>
  );
}
