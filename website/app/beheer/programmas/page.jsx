  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-3xl font-black text-ink">Programma's</h1>
        <Link href="/beheer/oefeningen" className="rounded-full bg-paper px-4 py-2 text-sm font-bold text-ink transition hover:bg-accent/15">≣ Oefeningenbibliotheek →</Link>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {zichtbaar.map((p) => (
          <Link key={p.id} href={`/beheer/programmas/${p.id}`} className="rounded-2xl border border-borderc bg-surface p-5 transition hover:border-accent">
        ...
        {zichtbaar.length === 0 && (
          <p className="text-sm text-ink/50">{zoek ? `Geen programma gevonden voor “${zoek}”.` : "Nog geen programma's."}</p>
        )}
      </div>
    </div>
  );
