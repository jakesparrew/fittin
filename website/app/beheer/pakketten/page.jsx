...
export default async function Pakketten() {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      <h1 className="text-3xl font-black text-ink">Pakketten &amp; abonnementen</h1>
      ...
      <div className="mt-6 space-y-4">
        {(packages || []).map((p) => (
          <ActionForm key={p.id} action={upsertPackage} success="Pakket opgeslagen ✓" className="grid items-end gap-3 rounded-2xl border border-borderc bg-surface p-5 md:grid-cols-[1.3fr_.8fr_.7fr_.7fr_.6fr_.6fr_auto]">
            ...
            <p className="text-xs text-ink/40 md:col-span-7">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {(packages || []).map((p) => (
          <form key={p.id} action={togglePackage}>
            ...
            <button className="rounded-full border-2 border-borderc px-4 py-2 text-xs font-bold text-ink transition hover:border-lav">
      ...
      <h2 className="mt-10 font-black text-ink">Nieuw pakket</h2>
      <ActionForm action={upsertPackage} success="Pakket opgeslagen ✓" className="mt-3 grid items-end gap-3 rounded-2xl border border-dashed border-borderc bg-surface p-5 md:grid-cols-[1.3fr_.8fr_.7fr_.7fr_.6fr_auto]">
...
function In({ name, type = "text", defaultValue }) {
  return <input name={name} type={type} defaultValue={defaultValue ?? ""} className="w-full rounded-xl border-2 border-borderc bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-accent" />;
}
