...
export default async function BeheerDashboard() {
  ...
  const [
    ...
    supabase.from("coach_session_requests").select("id", { count: "exact", head: true }).eq("status", "pending"),
    supabase.from("inbound_emails").select("id", { count: "exact", head: true }).eq("gym_id", gym.id).eq("archived", false),
  ...
  const actionCount = unpaid.length + (pendingReq || 0) + (unreadInbox || 0);
    ...
    <div className="px-4 py-6 md:px-8 md:py-8">
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <ActionCard href="/beheer/coaches" tone={pendingReq ? "warn" : "ok"} label="Coach-aanvragen" value={pendingReq || 0} sub={pendingReq ? "wachten op jou" : "niets open"} />
          <ActionCard href="/beheer/inbox" tone={unreadInbox ? "warn" : "ok"} label="Inbox" value={unreadInbox || 0} sub={unreadInbox ? "ongelezen" : "leeg"} />
          <ActionCard href="/beheer/leden?filter=atrisk" tone="neutral" label="Leden met risico" value="›" sub="al even niet geweest" />
        </div>
      </section>
...
function ActionCard({ href, label, value, sub, tone }) {
  const ring = tone === "warn" ? "border-amber-300 bg-amber-50" : tone === "ok" ? "border-borderc bg-white" : "border-borderc bg-white";
  const num = tone === "warn" ? "text-amber-600" : "text-brand";
  return (
    <Link href={href} className={"rounded-2xl border p-5 transition hover:-translate-y-0.5 hover:shadow-sm " + ring}>
      <p className="text-xs font-bold uppercase tracking-widest text-lav">{label}</p>
