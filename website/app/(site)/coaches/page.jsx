    ...
    <main className="bg-paper">
      ...
      <div className="mx-auto max-w-5xl px-5 py-16">
        ...
        <div className={`mt-10 grid gap-6 ${alleen ? "max-w-sm" : list.length === 2 ? "sm:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3"}`}>
          {list.map((c) => (
            <Link key={c.id} href={`/coaches/${coachSlug(c)}`} className="group overflow-hidden rounded-3xl border border-borderc bg-surface transition hover:-translate-y-1 hover:shadow-lg hover:shadow-brand/5">
              <div className="relative aspect-[4/3] bg-paper">
                {c.coach_photo_url ? (
                  ...
                  <div className="flex h-full items-center justify-center text-5xl font-black text-ink/15">{(c.full_name || "C").slice(0, 1)}</div>
                )}
              </div>
              <div className="p-5">
                <p className="text-lg font-black text-ink">{c.full_name || "Coach"}</p>
                ...
                {c.coach_bio && <p className="mt-2 line-clamp-2 text-sm text-ink/60">{c.coach_bio}</p>}
                <span className="mt-3 inline-block text-sm font-bold text-ink/60 transition group-hover:text-ink">Bekijk profiel →</span>
              </div>
            </Link>
          ))}
          {list.length === 0 && (
            <div className="rounded-3xl border border-dashed border-borderc bg-surface p-10 text-center sm:col-span-2 lg:col-span-3">
              <p className="font-semibold text-ink/70">Binnenkort stellen onze coaches zich hier voor.</p>
            </div>
          )}
        </div>
        ...
        <div className="mt-10 text-center">
          ...
          <p className="mt-3 text-sm text-ink/50">Gratis en vrijblijvend · we mailen je binnen 1 werkdag</p>
        </div>
      </div>
    </main>
  );
