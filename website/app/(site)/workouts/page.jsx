  ...
  return (
    <main className="min-h-screen bg-paper">
      ...
      <section className="mx-auto max-w-5xl px-5 py-10">
          ...
          {workouts.map((w) => {
            ...
            return (
              <Link
                ...
                className="group flex flex-col overflow-hidden rounded-3xl border border-borderc bg-surface transition hover:-translate-y-1 hover:shadow-xl hover:shadow-brand/5"
              >
                <div className="flex h-24 items-end p-5" style={{ background: `linear-gradient(135deg, ${c.color}22, #ffffff)` }}>
                  <span className="inline-flex items-center gap-2 rounded-full bg-surface/85 px-3 py-1 text-xs font-bold text-ink">
                    <span className="h-2 w-2 rounded-full" style={{ background: c.color }} />
                    {c.label}
                  </span>
                </div>
                <div className="flex flex-1 flex-col p-5">
                  <h2 className="text-lg font-black text-ink">{w.name}</h2>
                  <p className="mt-1 flex-1 text-sm text-ink/60">{w.subtitle}</p>
        ...
        {workouts.length === 0 && <p className="text-ink/50">Nog geen workouts beschikbaar.</p>}

        <div className="mt-10 rounded-3xl border border-borderc bg-surface p-6">
          <h3 className="font-black text-ink">Liever je eigen plan?</h3>
          <p className="mt-1 text-sm text-ink/60">Bouw je eigen workout uit de oefeningenbibliotheek.</p>
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <Link href="/oefeningen" className="rounded-full border-2 border-borderc px-5 py-2.5 text-sm font-bold text-ink transition hover:border-accent">Oefeningen</Link>
          </div>
        </div>
      </section>
    </main>
