...
export default function QuickStart({ title = "Snelstart", steps = [], defaultOpen = false }) {
  return (
    <details open={defaultOpen} className="mt-4 rounded-2xl border border-accent/30 bg-accent/5 p-4">
      <summary className="cursor-pointer text-sm font-black text-ink">💡 {title} — zo werkt het</summary>
      <ol className="mt-3 space-y-2">
        {steps.map((s, i) => (
          <li key={i} className="flex gap-3 text-sm text-ink/70">
            ...
            <span><span className="font-bold text-ink">{s.title}</span>{s.body ? ` — ${s.body}` : ""}</span>
          </li>
        ))}
      </ol>
    </details>
