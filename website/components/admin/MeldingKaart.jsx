      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-surface px-2.5 py-0.5 text-xs font-black text-ink">{catEmoji(r.category)} {catLabel(r.category)}</span>
            <span className="font-black text-ink">{r.member?.full_name || "Lid"}</span>
            <span className="text-xs font-semibold text-ink-soft">{fmt(r.created_at)}</span>
            {gezien && <span className="rounded-full bg-surface px-2.5 py-0.5 text-[10px] font-bold text-accentdark">👀 gezien</span>}
          </p>
          {r.message && <p className="mt-2 text-sm leading-relaxed text-ink/80">{r.message}</p>}
          ...
          {vorige && (
            <p className="mt-2 rounded-lg bg-surface px-3 py-2 text-xs text-ink-soft">
              Vorige gebruiker van de zaal: <strong className="text-ink">{vorige.naam}</strong> — {fmt(vorige.starts_at)}.
              <span className="block text-[11px] opacity-70">Alleen jij ziet dit.</span>
            </p>
          )}

          {fotoUrl && (
            <a href={fotoUrl} target="_blank" rel="noreferrer" className="mt-3 block w-fit overflow-hidden rounded-xl border border-borderc bg-surface">
        ...
        {!gezien && (
          <ActionForm action={bevestigMelding} success="Het lid weet dat je kijkt ✓" className="shrink-0">
            ...
            <button className="rounded-full bg-surface px-4 py-1.5 text-xs font-bold text-ink transition hover:bg-brand hover:text-white">👀 Ik kijk ernaar</button>
          </ActionForm>
        )}
      </div>
        ...
        <ActionForm action={zetZaalNotitie} success="Bijgewerkt ✓">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-lg border-2 border-amber-200 bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-amber-400"
            />
            <button className="shrink-0 rounded-full bg-surface px-3 py-2 text-xs font-bold text-ink">Bewaar</button>
          </div>
