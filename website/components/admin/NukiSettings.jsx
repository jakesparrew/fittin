...
export default function NukiSettings({ initial, tokenSet, envToken, readOnly }) {
  ...
  return (
    <form action={action} className="mt-8 max-w-2xl space-y-5 rounded-2xl border border-borderc bg-surface p-7">
      <div>
        <h2 className="text-xl font-black text-ink">🔐 Nuki deurslot</h2>
      ...
      <label className="flex items-center gap-3 rounded-xl bg-paper p-3 text-sm font-semibold text-ink">
      ...
      <label className="block">
        <span className="mb-1 block text-sm font-bold text-ink">Nuki Web API-token</span>
        <input
          name="nuki_api_token"
          type="password"
          autoComplete="off"
        ...
        />
        <span className="mt-1 block text-xs text-ink/45">Maak een token aan in Nuki Web → Account → API. Wordt versleuteld op de server bewaard, nooit getoond.</span>
      </label>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button type="button" onClick={runTest} disabled={readOnly || testing} className="rounded-full border-2 border-borderc px-6 py-3 font-bold text-ink transition hover:border-accent disabled:opacity-40">
      ...
      {test?.ok && (
        <div className="rounded-xl bg-accent/10 p-3 text-sm text-ink/80">
