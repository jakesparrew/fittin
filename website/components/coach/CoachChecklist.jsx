    ...
    <section className="mt-6 rounded-3xl border-2 border-accent/40 bg-accent/5 p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-black text-ink">Maak je profiel compleet ({done}/{steps.length})</h2>
          <p className="mt-1 text-sm text-ink/60">Zo verschijn je op de website en kunnen leden je vinden en boeken.</p>
          {/* Progress bar — a visible sense of "how far am I". */}
          <div className="mt-3 h-2 w-48 max-w-full overflow-hidden rounded-full bg-surface">
            <div className="h-full rounded-full bg-accent transition-all ease-uit" style={{ width: `${pct}%` }} />
          </div>
        </div>
        <button onClick={dismiss} className="rounded-full px-3 py-1.5 text-xs font-bold text-ink/40 transition hover:text-ink" title="Even verbergen — komt terug tot je profiel compleet is">Even verbergen</button>
      </div>
      <ol className="mt-4 space-y-2">
        {steps.map((s) => (
          <li key={s.label} className="flex items-center gap-3 rounded-2xl border border-borderc bg-surface p-3">
            <span className={"flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-black " + (s.done ? "bg-accent text-brand" : "bg-paper text-ink/40")}>{s.done ? "✓" : ""}</span>
            <div className="min-w-0 flex-1">
              <p className={"text-sm font-bold " + (s.done ? "text-ink/50 line-through" : "text-ink")}>{s.label}</p>
              {!s.done && s.hint && <p className="text-xs text-ink/50">{s.hint}</p>}
            </div>
            {!s.done && (
