...
export default function ShareReferral({ code, compact = false }) {
  ...
  // Altijd het publieke adres — ook als dit in de app of op een preview-deploy draait.
  ...
  const copy = async () => {
    // Native share sheet in the app and on mobile; clipboard on desktop (lib/native/share).
    const r = await deel({ title: "Train mee bij Fittin'", text: "Je boekt de héle zaal privé — jouw eerste uur is gratis 💪", url });
    if (r === "copied") {
      setCopied(true);
      setTimeout(() => setCopied(false), 2500);
    }
    if (r === "shared" || r === "copied") track("referral_link_shared");
  };
  ...
  return (
    <div className={compact ? "flex flex-wrap gap-2" : "flex flex-wrap items-center gap-2"}>
      ...
      <a
        ...
        onClick={whatsapp}
        className="inline-flex items-center gap-1.5 rounded-full border-2 border-borderc bg-surface px-4 py-2 text-sm font-bold text-ink transition hover:border-accent"
      >
        WhatsApp
      </a>
    </div>
