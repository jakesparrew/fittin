  ...
  return (
    <div className="mt-4 space-y-2">
      ...
      <div className="relative">
        ...
        {results.length > 0 && (
          <div className="absolute z-20 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border border-borderc bg-surface shadow-lg">
            {results.map((m) => (
              <button key={m.id} type="button" onClick={() => pick(m)} className="block w-full px-3 py-2 text-left text-sm text-ink transition hover:bg-paper">{m.name}</button>
            ))}
          </div>
        )}
      </div>
      ...
      <details className="pt-1 text-sm">
        <summary className="cursor-pointer text-ink/50">Vriend nog geen lid? Nodig uit via e-mail</summary>
