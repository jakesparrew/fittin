  ...
  return (
    <section className="mt-8 rounded-3xl border border-borderc bg-surface p-6">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h2 className="font-black text-ink">AI-coach</h2>
      ...
      {plan.doorverwezen_at && (
        <div className="mt-3 rounded-2xl border-2 border-amber-300 bg-amber-50 p-4">
          <p className="text-sm font-black text-ink">De AI-coach verwees door naar jou</p>
      ...
      {plan.samenvatting && <p className="mt-3 text-sm leading-relaxed text-ink/75">{plan.samenvatting}</p>}

      {!gehad.length ? (
        <p className="mt-3 text-sm text-ink/50">Er staat nog geen week open.</p>
      ) : (
          ...
          {gehad.slice(0, 8).map((w) => {
            ...
            return (
              <li key={w.id} className="border-l-2 border-borderc pl-4">
                ...
                {w.weekanalyse && <p className="mt-1 text-sm leading-relaxed text-ink/75">{w.weekanalyse}</p>}
                {c && (
                ...
                {c?.vrij && <p className="mt-1 text-xs italic text-ink/55">&ldquo;{c.vrij}&rdquo;</p>}
              </li>
            );
          })}
        </ul>
